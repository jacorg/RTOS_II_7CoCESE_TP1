...
void Task_OP4( void* taskParmPtr ){
	...
	while(1){
		...
		Frame_parameters.Token->t_InitConvert = cyclesCounterToUs(DWT_CYCCNT);
		packetToUpper((Frame_parameters.Token->PayLoad));
		Frame_parameters.Token->t_EndConvert = cyclesCounterToUs(DWT_CYCCNT);
		// Enviar a cola de TaskTxUARt
		ModuleDinamicMemory_send2(&ModuleData,&Frame_parameters,0,NULL,NULL, xPointerQueue_3,portMAX_DELAY);
	...
	while(true){
		...
		if( uartTxReady( UART_USB ) ){
			...
			if(Frame_parameters.Operation != OP4){
				...
				sprintf( Txbuffer, " teTx:%lu",(Frame_parameters.Token->t_EndTx = cyclesCounterToUs(DWT_CYCCNT)));
				uartWriteString(UART_USB,Txbuffer);
				Frame_parameters.Token->Id_de_paquete ++;
			}
		...
		if(Frame_parameters.Token != NULL) ModuleData.MemoryFreeFunction((Frame_parameters.Token)); //TOKEN 10.6.19
	}
}
